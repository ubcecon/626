---
title: "Probability"
author: "Paul Schrimpf"
date: last-modified
format:
  revealjs:
    theme: moon
    smaller: true
    min-scale: 0.1
title-slide-attributes:
  data-background-image: "ratney.jpg"
  data-background-size: contain

bibliography: ../626.bib
#engine: julia
---


## Reading

- **@song2021 chapter 2** (which is the basis for these slides)
- @spanos2019 chapters 2 & 3
- @pollard2002


$$
\def\Er{{\mathrm{E}}}
\def\R{{\mathbb{R}}}
$$

# Probability


> Mathematically, a probability space is a special measure space where the measure has total mass one. But, our attitude and emotional response toward one is entirely diﬀerent from those toward the other. On a measure space everything is deterministicand certain, on a probability space we face randomness and uncertainty.

@cinlar2011

## Probability Space

::: {.callout-tip icon=false}

### Definitions

- Given a measure space $(\Omega ,\mathscr{F})$, a **probability** (or **probability measure**)$\ P$ is a
measure s.t. $P(\Omega )=1$
-  $(\Omega ,\mathscr{F}, P)$ is a **probability space**
- $\Omega$ is a **sample space**
- $\omega \in \Omega$ is an **outcome**
- $A \in \mathscr{F}$ is an **event**

:::

::: {.notes}

Emphasize that development of probability was motivated by practical problems.

Give an example with coins or dice.

Events needed both because intrinsically interesting and to describe measurability.

:::


---

::: {.callout-note icon=false}

### Exercise

Show $\forall A, B \in \mathscr{F}$

1. $P(\varnothing )=0$

2. $P(A)\leq 1.$

3. $P(A^{c})=1-P(A)$

4. $P(A\cup B)=P(A)+P(B)-P(A\cap B)$

5. If $A\subset B$, then $P(A)\leq P(B)$

6. $P(B\cap A^{c})=P(B)-P(A\cap B)$

:::

# Random Variables

## Random Variable

::: {.callout-tip icon=false}

### Definition

A **random variable** $X$ is a measurable function from $\Omega$ to $\R$

:::

::: {.notes}

A sample space can be a set of anything. Most analysis will be done with numbers, so we need random variables.

:::

## Distribution

::: {.callout-tip icon=false}

### Definition

Let $(\Omega ,\mathscr{F},P)$ be a probability space, $X$ a random variable on $(\Omega ,\mathscr{F})$.
A **distribution** $P_{X}$ induced by $X$ is a probability measure on $(\R,\mathscr{B}(\R))$ such
that : $\forall B\in \mathscr{B}(\R)$,
$$
P_{X}(B)\equiv P\left\{ \omega \in \Omega :X(\omega )\in B\right\}
$$

:::

::: {.notes}

In many applications, the underlying probability space $(\Omega, \matscr{F}, P)$ will mostly sit in the background and most calculations will be done with $(\R, \mathscr{B})(\R), P_X)$.

Given an example.

:::

## Distribution Function

::: {.callout-tip icon=false}

### Definition

The cumulative distribution function (CDF) of a random variable $X$ with
distribution $P_{X}$ is defined to be a function
$F_X:\R\rightarrow [0,1]$ such that
$$
F_X(t)=P_{X}\left( (-\infty ,t]\right) .
$$

:::

::: {.notes}

By definition, we can go from $P_X$ to $F_X$. By Carathéodary's extension theorem, we can do the reverse. Either the CDF or $P_X$ completely characterizes the distribution.

:::


## Density

::: {.callout-tip icon=false}

### Definition

(i) Let $X$ be a random variable with
distribution $P_{X}$. When $P_{X}\ll \lambda$, we call $X$ a
**(absolutely) continuous random variable**, and call the Radon-Nikodym derivative
$f_X\equiv dP_{X}/d\lambda$ the (probability) **density function** of $P_{X}$.

(ii) We say that $X$ is a **discrete random variable**, if
there exists a countable set $A\subset \R$ and such that
$P_{X}A^{c}=0$

:::

::: {.notes}

Density wrt to other measures? e.g. dirac?

:::

---

::: {.callout-note icon=false}

### Exercise

Show that when $X$ is continuous, its CDF is a continuous function

:::

::: {.notes}

$X$ being continuous implies that $P_X(\{a\}) = 0$
for all $a \in \R$ since $\lambda(\{a\}) = 0$. By continuity of measure,
$P_{X}((-\infty, a]) = \lim_{\epsilon \downarrow 0} P_{X}((-\infty, a+\epsilon])$, so
$$F_X(a) = \lim_{\epsilon \downarrow 0} F_X(a+\epsilon).$$
Similarly,
$$F_x(a) = \lim_{\epsilon \uparrow 0} F_X(a-\epsilon).$$

:::

<!--- TODO: add examples or uses to go with these definitions --->

# Expectation

::: {.callout-tip icon=false}

### Definition

The **expectation** of $X$ is $\Er [X] = \int_\Omega X(\omega) dP(\omega)$

:::

- $\Er [X] = \int_{\R} x dP_X(x)$
- positive semi-definite: $X \geq 0$ implies $\Er [X] \geq 0$
- linear : $\Er[aX + bY] = a \Er [X] + b \Er [Y]$

::: {.notes}

If $X$ is continuous, i.e. $P_X << \lambda$ where $\lambda$ is Lebesgue measure, then there is a Radon-Nikodym derivative, $f_x$, and we can write the expectation as
$$
\Er X = \int_{R} x f_x(x) d\lambda(x).
$$


For any measurable function $g$, we can define $\mathcal{E}_X: \{measurable functions\} \to \R$ by $\mathcal{E}_X(g) = \Er[g(X)]$. We could call this the expectation functional of $X$.  Taking $g = 1\{X \leq x\}$ for $x \in \R$, we can recover the CDF, $F_X(x) = \mathcal{E}_X(1\{X \leq x \})$. Thus, the expectation functional is a third way, along with the CDF and probability measure, to describe the distribution of $X$.

:::


# Inequalities


## Markov's

::: {.callout-warning icon=false}

### Markov's Inequality

$P(|X|>\epsilon) \leq \frac{\Er[|X|^k]}{\epsilon^k}$
$\forall \epsilon > 0, k > 0$
:::

::: {.notes}

Follows from $1\{|X|>\epsilon\} \leq \frac{|X|^k}{\epsilon^k}$.

Example to get LLN.

:::

## Jensen's

::: {.callout-warning icon=false}

### Jensen's Inequality

Suppose that $g$ is convex and $X$ and $g(X)$ are integrable, then
$g(\Er X) \leq \Er[g(X)]$

:::

::: {.callout-note icon=false}

### Exercise

Show $\Er[|X|^p] \leq \left(\Er[|X|^q] \right)^{p/q}$ for all $0 < p \leq q$.

:::

::: {.notes}

To show Jensen's inequality note that convexity implies that for any
$x_0$, $\exists c$ s.t. $g(x) \geq g(x_0) + c(x-x_0)$

:::

## Cauchy-Schwarz

::: {.callout-warning icon=false}

### Cauchy-Schwarz Inequality

$\left(\Er[XY]\right)^2 \leq \Er[X^2] \Er[Y^2]$

:::

::: {.notes}

Proof:

- For any $\lambda \neq 0$, $\Er[(\lambda X - \lambda^{-1} Y)^2] \geq 0$, so
- $2\Er[XY] \leq \lambda^2 \Er[X^2] + \lambda^{-2} \Er[Y^2]$
- set $\lambda^2 = \sqrt{\Er[Y^2]/\Er[X^2]}$

:::

# Dependence and Information

## Generated $\sigma$-field

- $\sigma(X)$ is $\sigma$**-field generated by** $X$
  - smallest $\sigma$-field w.r.t. which $X$ is measurable
  - $\sigma(X) = \{X^{-1}(B): B \in \mathscr{B}(\R)\}$

::: {.notes}

Explain some simple example(s).


:::

## Information

- $\forall E \in \sigma(X)$, observing value $x$ of $X$, tells us whether $E$ occurred
- if $\sigma(X_1) \subset \sigma(X_2)$, then $\sigma(X_2)$ **has more information** than $\sigma(X_1)$

::: {.notes}

Example 4.3 in @song2021.

:::

---

::: {.callout-note icon=false}

### Exercise 4.7

Suppose $g:\R \to \R$ is Borel measurable, then show $\sigma(g(X))
\subset \sigma(X)$


:::

## Dependence

::: {.callout-warning icon=false}

### Theorem 4.2

Suppose $\sigma(W) \subset \sigma(X)$, then $\exists$ Borel measurable $g$ s.t. $W=g(X)$

:::

::: {.notes}

Proof in @cinlar2011 chapter 2, section 4.

:::

# Independence


## Independence

::: {.callout-tip icon=false}

### Definition

- Events $A_1, ..., A_m$ are **independent** if for any sub-collection
$A_{i_1}, ..., A_{i_s}$
$$
P\left(\cap_{j=1}^s A_{i_j}\right) = \prod_{j=1}^s P(A_{i_j})
$$
- $\sigma$-fields, $\mathscr{F}_1, .., \mathscr{F}_m \subset
\mathscr{F}$ are **independent** if for any $\mathscr{F}_{i_1}, ..,
\mathscr{F}_{i_s}$ and $E_j \in \mathscr{F}_j$,
$$
P\left(\cap_{j=1}^s E_{i_j}\right) = \prod_{j=1}^s P(E_{i_j})
$$
- Random variables $X_1, ..., X_m$ are **independent** if
$\sigma(X_1), ..., \sigma(X_m)$ are independent

:::

- $X_1$ and $X_2$ are independent iff
$F_{X_1,X_2}(x_1,x_2) = F_{X_1}(x_1) F_{X_2}(x_2)$ for all $x_1, x_2$.

::: {.notes}

Show some simple example(s).

:::



## Random Vectors

- measurable $X: \Omega \to \R^n$
- $\sigma(X) = \{X^{-1}(B): B \in \mathscr{B}(\R^n)\} =$ smallest
$\sigma$-field containing $\cup_{i=1}^n \sigma(X_i)$

::: {.callout-warning icon=false}

### Theorem

Suppose that $X=(X_1, X_2)$ and $Y=(Y_1, Y_2)$ are independent, then
$g(X)$ and $h(Y)$ are independent

:::

::: {.notes}

As promised, an easy way to show obvious independence without any tedious calculations.

:::


# Conditioning

## Conditional Distribution

- $X,Y$ random variales with cdf $F$
- If $P(X=x)>0$, then conditional cdf is
$$
F_{Y|X}(y|x) = P(Y\leq y | X=x) = \frac{P(Y \leq y \cap X=x)}{P(X=x)}
$$
- If $X$ is continuous, then
$$
F_{Y|X}(y|x) = \lim_{\epsilon \downarrow 0} P(Y \leq y | x-\epsilon \leq X \leq x+\epsilon) = \frac{\partial F/\partial x(x,y)}{f_X(x)}
$$

::: {.notes}

Proof of last equality:
$$
\begin{align*}
P(Y \leq y | x-\epsilon \leq X \leq
x+\epsilon) = & \frac{ P(Y \leq y \cap x-\epsilon \leq X \leq x+\epsilon
  )}{P(x-\epsilon \leq X \leq x+\epsilon)} \\
  = & \frac{F(y, x+\epsilon) - F(y,x-\epsilon)}{F_X(x+\epsilon) - F_X(x-\epsilon)}
\end{align*}
$$
Divide numerator and denominator by $2\epsilon$, apply l'Hospital's rule, and definition of derivative.

:::

## Conditional Density

- $f_{Y|X}(y|x) = \frac{\partial F_{Y|X}(y|x)}{\partial y} = \frac{f(x,y)}{f_x(x)}$

## Conditional Expectation

- For continuous $Y$,
$$
\Er[Y|X=x] = \int y f_{Y|X}(y|x) dy
$$


## Conditional Expectation

- Previous definition needs to be stated separately for continuous vs discrete
- Previous definition leads to ambiguity --- how you take limits toward $X=x$ matters (Borel paradox)
- Measure theoretic definition avoids these problems, but is more abstract

## Conditional Expectation

::: {.callout-tip icon=false}

### Definition

Let $\mathscr{G} \subset \mathscr{F}$ be $\sigma$-fields, $Y$ a random
variable with $\Er |Y| < \infty$, then the **conditional expectation**
of $Y$ given $\mathscr{G}$ is $\Er[Y|\mathscr{G}](\cdot): \Omega \to \R$ s.t.

1. $\Er[Y|\mathscr{G}](\cdot)$ is $\mathscr{G}$ measurable

2. $\int_A \Er[Y|\mathscr{G}] dP = \int_A Y dP$ $\forall A \in \mathscr{G}$

:::

::: {.incremental}

- Existence from Radon-Nikodym theorem
- $\Er[Y|X] \equiv \Er[Y|\sigma(X)]$

:::

## Conditional Expectation

::: {.callout-note icon=false}

### Exercise

1. If $X$ and $Y$ are discrete with support $\{x_i\}_{i=1}^I \times \{y_j\}_{j=1}^J$ and PMF $p$
 then
 $$
 \Er[Y|X=x_i] = \frac{\sum_{j=1}^J y_j p(x_i,y_j)} {\sum_{j=1}^J p(x_i,y_j)}
 $$

2. If $X$ and $Y$ are continuous with density $f$, then
$$
\Er[Y|X=x] = \frac{\int y f(x,y) dy}{\int f(x,y) dy}
$$

:::

::: {.notes}

1. Can be shown explicitly. For any $\{E_k\}_{k=1}^m$ partition of $\Omega$, let
$\mathscr{G} = \sigma(\{E_k\}_{k=1}^m)$
   - $\Er[Y | \mathscr{G}](\omega) = \sum_{k=1}^m c_k 1\{\omega \in E_k\}$, then use 2 from definition to solve for $c_k$.
   - Take the partition to be $\{x_i\} \times \mathcal{Y}$

2. This is shown by verifying that this function satisfies (2).

:::


## Properties of Conditional Expectation

::: {.incremental}

- If $X$ is $\mathscr{G}$ measurable, then $\Er[XY| \mathscr{G}] = X \Er[Y|\mathscr{G}]$ a.e.
- If $\sigma(X) \subset \sigma(Z)$, then $\Er[XY|Z] = X \Er[Y|Z]$
- If $\sigma(X) \subset \sigma(Z)$, then $\Er[\Er[Y|Z]|X] = \Er[Y|X]$
- If $Y$ and $X$ are independent, then $\Er[Y | X ] = \Er[Y]$

:::

::: {.notes}

One way to prove these statements is to first show them for indicator functions, then approximate $X$ by a sequence of simple functions, finally apply a convergence theorem to get the result. The notes include such a proof for the last claim about independent variables.

:::

## $\Er[Y|\mathscr{G}]$ as Orthogonal Projection

::: {.callout-warning icon=false}

### Theorem

Let $(\Omega, \mathscr{F}, P)$ be a probability space, $\mathscr{G}$ a sub $\sigma$-field,
then for any
$Y \in \mathcal{L}^2(\Omega, \mathscr{F}, P) = \{X: \Omega \to \mathbb{R} \text{ s.t. } X \text{ }\mathscr{F}\text{-measurable, } \int X^2 dP < \infty \}$,
$$
\inf_{W \in \mathcal{L}^2(\Omega, \mathscr{G}, P)} \Er[(Y-W)^2] = \Er[ (Y - \Er[Y | \mathscr{G}])^2]
$$

:::



## Conditional Measure

::: {.callout-tip icon=false}

### Definition

Let $\mathscr{G}$ be a sub $\sigma$-field of $\mathscr{F}$. The **conditional probability measure**
given $\mathscr{G}$ is defined to be a map $P(\cdot \mid \mathscr{G})(\cdot ):\mathscr{F}\times \Omega \rightarrow [0,1]$ such that

1. For each $A\in \mathscr{F}$, $P(A \mid \mathscr{G})(\cdot )=\mathbf{E}\left[ 1\{\omega \in A\} \mid \mathscr{G}\right] (\cdot )$, a.e.

2. for each $\omega \in \Omega$, $P(\cdot \mid \mathscr{G})(\omega )$ is a probability measure on $(\Omega ,\mathscr{F}).$

:::

## Conditional Independence

::: {.callout-tip icon=false}

### Definition

1. Events $A_1, ..., A_m \in \mathscr{F}$ are **conditionally independent** given $\mathscr{G}$ if for any sub-collection,
$$
P\left( \cap_{j=1}^s A_{i_j} | \mathscr{G} \right) = \prod_{j=1}^s P(A_{i_j} | \mathscr{G})
$$

2. Sub $\sigma$-fields $\mathscr{F}_1, ..., \mathscr{F}_m$ are
**conditionally independent** given $\mathscr{G}$ if for any
sub-collection and events, $E_i \in \mathscr{F}_i$,
$$
P\left( \cap_{j=1}^s E_{i_j} | \mathscr{G} \right) = \prod_{j=1}^s P(E_{i_j} | \mathscr{G})
$$

3. Random variables $X_1, ..., X_m$ are **conditionally independent**
 given $\mathscr{G}$ if $\sigma(X_1), ..., \sigma(X_m)$ are conditionally independent given $\mathscr{G}$


:::


# References
