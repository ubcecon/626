...
$$
    ...
    - $y= X\beta + u$ fits setup with $L = \mathcal{R}(X)$
:::
...
If $\Er[u] = 0$ and $\Er[uu'] = \sigma^2 I_n$, then the best linear unbiased estimator (BLUE) of $a'\theta = $a'\hat{\theta}$
where $\hat{\theta} = P_L y$
