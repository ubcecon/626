...
- If $f \geq 0$, define
$$
\int fd\mu =\sup_{f_n \text{ simple, bounded+}} \int f_{n}d\mu
$$
...
- Measure $\vu$ is **absolutely continuous** with respect to $\mu$ if
...
If $f_n:\Omega \to \mathbf{R}$ are measurable, $f_{n}\geq 0$, then $\int \left( \text{liminf}_{n\rightarrow \infty }f_{n}d\mu \right)
\leq $liminf$_{n\rightarrow \infty }\int f_{n}d\mu $
