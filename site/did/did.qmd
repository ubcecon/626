...
```{julia}
m=reg(dfc, @formula(y ~ -1 + cohort*ct + fe(id)), Vcov.cluster(:id))
regtable(m, render=AsciiTable())
```
...
```{julia}
function stackeddata(dfc, groupvar=:cohort, controlgroup=1)
    dfs = DataFrame()
    for g in unique(dfc[!,groupvar])
        if (g == controlgroup) continue end
        dfg = dfc[(dfc[!,groupvar] .== g) .| (dfc[!,groupvar] .== controlgroup), :]
        dfg = sort(dfg, [:id, :t])
        dfg[!,:rtt] = dfg.t .- minimum(dfg.t[(dfg[!,groupvar] .== g) .& (dfg.D .== true)]) # time relative to treatment
        dfg[!,:group] .= g
        dfs = vcat(dfs, dfg)
    end
    dfs.rtt = categorical(dfs.rtt)
    dfs.group = categorical(dfs.group)
    dfs.evertreated = dfs[!,groupvar] .!= controlgroup
    return(dfs)
end

dfs = stackeddata(dfc)

m = reg(dfs, @formula(y ~ -1 + rtt*group + fe(id) + fe(t)))
regtable(m, render=AsciiTable())
```
