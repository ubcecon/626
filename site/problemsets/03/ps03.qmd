...
In the linear model, $Y = \alpha + \beta X + \epsilon$, assume $\var(X)>0$ and $\Er[\epsilon] = 0$. Show that without any more assumptions, $\beta$ is not identified by finding values of $\beta$ that are observationally equivalent.
