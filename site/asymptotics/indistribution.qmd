...
$$
\Er\left[\varphi(Z_{n,n}) - \varphi(Z_{n,0}) \right] = - \sum_{i=0}^{n-1} \Er\left[ \varphi(Z_{n,i}) - \varphi(Z_{n,i+1})\right]
$$
...
$$
\left\vert \Er\left[ \varphi(Z_{n,i}) - \varphi(Z_{n,i+1})\right] \right\vert \leq \left( \frac{\Er|G_{i+1}|^3 + \Er|X_{i+1}|^3}{n^{3/2}} \sup \varphi'''(x) \right)
$$
so for some constant $C$ that depends on $\varphi$, but not on $n$ or $i$. We can conclude that
$$
\Er\left[\varphi(Z_{n,n}) - \varphi(Z_{n,0}) \right] \leq n \frac{\Er|G_{i+1}|^3 + \Er|X_{i+1}|^3}{n^{3/2}}
$$
...
^[See @raic2019 for details and @cck2023 for a similar result in high dimensions.]
...
### Multivariate Berry-Esseen Theorem
If $X_i \in \R^d$ are independent with $\Er[X] = 0$,
$\var\left(\frac{1}{\sqrt{n}} \sum_{i=1}^n X_i \right)=\Sigma$ with
$diagonal(\Sigma) = (1, ..., 1)'$,
$\Vert X_i \Vert_{\infty} \leq B_n$,
and $Z \sim N(0,\Sigma)$, then
$$
\sup_{A \subset \R^d, \text{rectangle}} \left\vert
P\left(\frac{1}{\sqrt{n}} \sum_{i=1}^n X_i \in A \right) - P(Z \in A) \right\vert \leq
C B_n \sqrt{\frac{\log(d)^3}{n \lambda_{min}(\Sigma)}} \log(n)
$$

:::

- See @cck2023 for this and similar results under different assumptions about $\Sigma$ and boundedness of $X_i$
