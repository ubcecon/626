...
- Supplementary: @molinari2020 , @matzkin2013, @imbens2020
...
# Descriptive Intrepretation of Population Estimator
...
- Advocated by Pearl, e.g. @pearl2015, @pearl2018
...
\begin{tikzpicture}[-latex, auto, node distance=1.5cm and 2cm, on grid, thick, scale=10,
                    obs/.style={circle, draw,black, text=black}]
\node[obs](D){$D_i$};
\node[obs] (X)[below=of D]{$X_i$};
\node[obs] (Y)[right=of D]{$Y_i$};
\node(E)[below=of Y]{$\epsilon_i$};
\node(U)[left=of D]{$U_i$};
\path[->] (X) edge (Y);
\path[->] (X) edge (D);
\path[->] (D) edge (Y);
\path[->](U) edge[dashed] (D);
\path[->] (E) edge[dashed] (Y);
\end{tikzpicture}
```
