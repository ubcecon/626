...
- We have an **economic model** that describes how the world works. This model has **parameters** we care about ($\theta_0$).
- We have **data**, which we view as a sample from some probability distribution ($P_X$).
...
- Identification is a conceptual problem about what is possible with infinite data.
- It is distinct from **estimation**, which is the problem of how to learn about $\theta_0$ from a finite sample.
- Identification must come before estimation.
...
The motivating idea here is we observe data. We do not know exactly the distribution of the data, $P_X$, but we are willing to assume it comes from some collection of distributions, $\mathcal{P}$. For example, suppose the data is the result of flipping a possibly biased coin $n$ times. Then we could have
$$
\mathcal{P} = \{ (\{0,1\}, 2^\{0,1\}, P(1) = p )^n : p \in [0,1] \}
$$
where the probability space to the $n$ means the product space as constructed in the last question on problem set 1.

$\theta_0$ is some parameter of interest related to $P_X$. In the example, there is not for $\theta_0$ to be other than $p$, but in general $\theta_0$ could be some aspect of $P_X$ that we especially care about instead of a complete description of it.
...
In this example, the probability model, $\mathcal{P}$, is a very large collection of distributions. This constrasts with the previous, biased-coin flipping, example where we could describe each element of $\mathcal{P}$ with a single number.
...
- Is $u \sim N(0,1)$ innocuous?
  ...
  - Suppose $\theta_0 = \int \frac{\partial E[Y|X=x]}{dx} dP_X(x)$ (the average marginal effect or average derivative)

:::

::: {.notes}
As, an aside, what the heck does $\frac{\partial E[Y|X=x]}{dx}$ even mean?  $E[Y|X=x]$ is only unique almost everywhere, so we could pick a version that is not differentiable. One approach to just declare that we always work with the smoothest version of $E[Y|X=x]$ and hope everything is okay. This mostly works in practice. But, the technically correct construction is that $\frac{\partial E[Y|X=x]}{dx}$ is a *weak derivative*, i.e. it satisfies an integration by parts formula. Suppose $f$ is differentiable in the usual sense with $f(a)=f(b)=0$, then integration by parts says
$$
\int_a^b E[Y|X=x] \frac{df}{dx}(x) dP_X(x) = \int_a^b \frac{\partial E[Y|X=x]}{dx} f(x) dP_X(x)
$$
We define $\frac{\partial E[Y|X=x]}{dx}$ as the unique (almost everywhere) function that satisfies this equation for all such $f$.

Anyway, some calculation will show that the average marginal effect is
unaffected by change from $N(0,1)$ to $N(\mu, \sigma^2)$.
...
- Identification can be defined without explicit mapping from data to parameters, e.g. @hsiao1983, @matzkin2007 by using observational equivalence
- Especially useful for showing non-identification

## Structures

- Be explicit about how distribution of data relates to an underlying economic model
- Set of model structures $S$
- Mapping from structure to distribution of observed data $P(\cdot;s)$
$$
\mathcal{P} = \{ P(\cdot; s): s \in S \}
$$
